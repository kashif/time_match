...
class SDE(nn.Module):
    ...
    # Drift
    def f(self, t, x_t):
        t = torch.ones(x_t.shape[0]).to(x_t) * t.clamp(0., 1.)
        drift = self.drift(x_t.unsqueeze(1), t, cond=self.cond).squeeze(1)
        return drift
...
class SIREGModel(nn.Module):
    ...
    def __init__(
        ...
        self.drift = UNet1DConditionModel(
            target_dim=self.input_size, hidden_size=hidden_size
        )

        # self.drift = EpsilonTheta(target_dim=input_size, cond_dim=hidden_size)
    ...
    def train_loss(self, x_0, x_1, t, cond=None):
        t = t.view(-1, 1, 1)
        ...
        field = self.drift(inter, t.squeeze(), cond=cond)
        field_ = self.drift(inter_, t.squeeze(), cond=cond)

        # print(field.shape)
        # print((self.interpolant.gamma_(t) * z).shape)
        # print(z.shape)
        # print(self.interpolant.gamma(t).shape)
        # print((self.epsilon * z / (self.interpolant.gamma(t) + EPS)).shape)
        # print()
        # l = 0.5 * (field ** 2 + field_ ** 2).sum(dim=-1)
        # l -= (dt_inter * (field + field_)).sum(dim=-1)
        # l += (self.interpolant.gamma_(t) * z * (field_ - field)).sum(dim=-1)
        # l += (self.epsilon * z * (field - field_)).sum(dim=-1) / (self.interpolant.gamma(t)[:, 0] + EPS)

        # l = (field - dt_inter + self.interpolant.gamma_(t) * z) ** 2
        # l += (field_ - dt_inter - self.interpolant.gamma_(t) * z) ** 2

        l = F.smooth_l1_loss(
            field,
            dt_inter + self.interpolant.gamma_(t) * z - self.epsilon * z / (self.interpolant.gamma(t) + EPS),
            reduction="none",
        )
        l += F.smooth_l1_loss(
            field_,
    ...
    def sample(self,
        ...
        # t = torch.linspace(0., 1., 2).to(x_0)
        sde = SDE(self.drift, epsilon=self.epsilon, cond=cond)
        next_sample = manual_solver(sde, context, self.n_timestep).unsqueeze(1)
        return next_sample
    ...
    def forward(
        ...
        next_sample = self.sample(
                        repeated_past_target[:, -1, ...] + 0.1 * torch.randn_like(repeated_past_target[:, -1, ...]), 
                        repeated_outputs[:, -1:, ...]
                    )
        # next_sample = sdeint(sde, x_0, t, dt=1e-3)[-1].unsqueeze(1)
        ...
        for k in range(1, self.prediction_length):
            ...
            # next_sample = sdeint(sde, x_0, t, dt=1e-3)[-1].unsqueeze(1)
            next_sample = self.sample(
                            next_sample[:, -1, ...] + 0.1 * torch.randn_like(next_sample[:, -1, ...]), 
                            repeated_outputs
                        )
    ...
    def get_loss_values(self, rnn_outputs, loc, scale, context, target, observed_values):
        ...
        if self.start_noise:
            context = torch.randn_like(context)
        context += 0.1 * torch.randn_like(context)
        # Sample a random timestep for each sample in the batch
        timesteps = torch.rand(B * T).to(target)

        loss = self.train_loss(
            context.reshape(B * T, 1, -1),
            target.reshape(B * T, 1, -1),
            timesteps.reshape(B * T),
