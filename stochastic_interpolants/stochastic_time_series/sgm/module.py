...
class SGMModel(nn.Module):
    ...
    def __init__(
        ...
        self.diffusion = Diffusion(linear_start, linear_end)
    ...
    def get_loss_values(self, rnn_outputs, loc, scale, target, observed_values):
        ...
        xt, noise, std = self.diffusion.q_sample(
            timesteps.reshape(B * T, 1),
        ...
        loss = F.smooth_l1_loss(
            std.reshape(B, T, 1) * pred.reshape(B, T, -1),
            -noise.reshape(B, T, -1),
            reduction="none",
        )
