...
class SDE(nn.Module):
    ...
    def __init__(self, drift, epsilon, gamma, cond):
        super().__init__()
    ...
    # Diffusion
    def g(self, t, x_t):
        diff = torch.zeros_like(x_t) + self.epsilon * self.gamma(t) * 2.
...
class SIEPSREGModel(nn.Module):
    ...
    def train_loss(self, x_0, x_1, t, cond=None):
        ...
        l = F.smooth_l1_loss(
            field,
            dt_inter + self.interpolant.gamma_(t) * z - self.epsilon * z,
            reduction="none",
        )
        l += F.smooth_l1_loss(
            field_,
    ...
    def sample(self,
        ...
        sde = SDE(self.drift, epsilon=self.epsilon, gamma=self.interpolant.gamma, cond=cond)
