...
class FMModel(nn.Module):
    ...
    def psi_t(self, x, t):
        ...
        return (1 - (1 - self.sigma_min) * t) * noise + t * x, noise

    def train_loss(self, x, t, cond=None):
        t = t.view(-1, 1, 1)
        x_t, noise = self.psi_t(x, t)
        v_t = self.drift(x_t, t.squeeze(), cond=cond)
        target = (x - (1 - self.sigma_min) * noise)
    ...
    @torch.no_grad()
    def manual_solver(self, context, cond):
        dt = 1. / self.n_timestep
        x = context
        t = torch.zeros(x.shape[0]).to(x)
        for step in range(self.n_timestep):
            x += self.drift(x.unsqueeze(1), t, cond=cond).squeeze(1) * dt 
            t += dt
    ...
    def sample(self,
        ...
        next_sample = self.manual_solver(context, cond).unsqueeze(1)
        return next_sample
