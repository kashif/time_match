...
def get_folder_name(args):
    ...
    name = f'{name}/{args.data}/{args.interpolant}_{args.gamma}/{args.coupling}_{args.clip}/{args.seed}_{args.epsilon}'
    return name

def train_model_step(model, optimizer, source, target, batch_size, device):
...
def train(args):
    ...
    with open(f'{name}/log.txt', 'w') as f:
        for i in range(max_steps):
            l = train_model_step(model, optimizer_model, source, target, batch_size, device)
            if i < max_steps - 1000:
                if args.clip is None or args.clip > 0.:
