...
from .module import TimeBlendModel


class TimeBlendLightningModule(pl.LightningModule):
    """
    A ``pl.LightningModule`` class that can be used to train a
    ``TimeBlendModel`` with PyTorch Lightning.

    This is a thin layer around a (wrapped) ``TimeBlendModel`` object,
    that exposes the methods to evaluate training and validation loss.

    Parameters
    ----------
    model
        ``TimeBlendModel`` to be trained.
    lr
    ...
    def __init__(
        ...
        self.model = TimeBlendModel(**model_kwargs)
        self.lr = lr
