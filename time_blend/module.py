...
class TimeBlendModel(nn.Module):
    """
    Module implementing the TimeBlend model.
    ...
    def __init__(
        ...
        self.alpha_blend = AlphaBlend(dim=self.input_size, unet=self.unet)
    ...
    def unroll_lagged_rnn(
        ...
        return output, loc, scale, output, static_feat, new_state
    ...
    def forward(
        ...
        rnn_output, loc, scale, _, static_feat, state = self.unroll_lagged_rnn(
            feat_static_cat,
            feat_static_real,
            past_time_feat,
            past_target,
        ...
        next_sample = self.alpha_blend.sample(
    ...
    def loss(
        ...
        if future_only:
            # TODO Fix this case
            loss_values = self.alpha_blend.loss(
                # [B, C, D]
                past_target,
                rnn_ouputs,
                future_target_reshaped,
